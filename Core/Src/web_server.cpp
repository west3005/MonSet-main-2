...
void WebServer::handleConfig(uint8_t sn){
    ...
    // Файл не найден на SD — отдаём 503 вместо embedded fallback
    sendResponse(sn, 503, "text/plain",
        "config.html not found on SD card (0:/www/config.html)", 53);
}
...
void WebServer::handleIndex(uint8_t sn){
    ...
    // Файл не найден на SD — отдаём 503 вместо embedded fallback
    sendResponse(sn, 503, "text/plain",
        "index.html not found on SD card (0:/www/index.html)", 52);
}
