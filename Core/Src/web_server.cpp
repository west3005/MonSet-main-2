...
static const uint32_t s_index_html_len = 17677U;


static const uint32_t s_config_html_len = 29280U;
...
void WebServer::handleApiBackupDownload(uint8_t sn){
    ...
    // Stream backup file line-by-line to avoid large RAM buffer
    char hdr[192];
    // We don't know size upfront — use chunked-like streaming with Connection:close
    int hl=std::snprintf(hdr,sizeof(hdr),
        "HTTP/1.1 200 OK\r\n"
